...
lista_arquivos_selecionados = []

# Lista de polígonos (Beams)
...
def processar_arquivos_excel(lista_caminhos):
    for caminho in lista_caminhos:
        try:
            ...
            if 'Longitude (°)' in df.columns and 'Latitude (°)' in df.columns:
                ...
                caminho_novo_arquivo = caminho.replace('.xlsx', '_resultados.xlsx')
                df.to_excel(caminho_novo_arquivo, index=False)
                print(f"Resultados salvos em: {caminho_novo_arquivo}")
...
#===========================//=========================== 
                ...
                # Adiciona uma nova coluna 'Beam' ao DataFrame com o resultado da verificação
                df['Beam'] = df.apply(lambda row: verificar_coordenada((row['Long (°)'], row['Lat (°)'])), axis=1)
...
#===========================//===========================                 
            ...
            print(f"Erro ao processar as coordenadas do arquivo Excel '{caminho}': {str(e)}")

def abrir_arquivo_excel():
    global lista_arquivos_selecionados  # Indica que está usando a variável global
    # Janela para seleção de um arquivo Excel
    lista_caminhos = filedialog.askopenfilenames(filetypes=[("Arquivos Excel", "*.xlsx")])

    # Atualiza o campo de entrada com o caminho do arquivo selecionado
    entrada.delete(0, tk.END)
    entrada.insert(0, ";".join(lista_caminhos))  # Usando ponto e vírgula como delimitador (pode ser ajustado)

    # Chama a função para mostrar os arquivos selecionados
    mostrar_arquivos_selecionados(lista_arquivos_selecionados, lista_caminhos)

    # Define a variável resultado
    resultado.config(text="")

def mostrar_arquivos_selecionados(lista_arquivos_selecionados, novos_arquivos):
    # Adiciona os novos arquivos à lista de arquivos selecionados
    lista_arquivos_selecionados.extend(novos_arquivos)

    # Limpa a Listbox antes de atualizar
    listbox_arquivos.delete(0, tk.END)

    # Adiciona todos os arquivos à Listbox
    for caminho in lista_arquivos_selecionados:
        listbox_arquivos.insert(tk.END, caminho)

def processar_coordenadas():
    try:
        # Obtém a lista de caminhos dos arquivos Excel a partir do campo de entrada
        lista_caminhos = entrada.get().split(";")  # Usando ponto e vírgula como delimitador (ajuste conforme necessário)

        # Processa os arquivos Excel
        processar_arquivos_excel(lista_caminhos)

        resultado.config(text="Resultados processados com sucesso!")

    except Exception as e:
        resultado.config(text=f"Erro ao processar as coordenadas: {str(e)}")

def verificar_coordenada_manualmente():
    try:
        # Obtém a coordenada do campo de entrada
        coordenada_manual = entrada_manual.get().split(",")
        if len(coordenada_manual) == 2:
            x, y = map(float, coordenada_manual)
            indice_Beam_ = verificar_coordenada((x, y))
            resultado_manual.config(text=f"A coordenada manual está no Beam {indice_Beam_}.")
        else:
            resultado_manual.config(text="Coordenadas inválidas. Use formato x,y.")
...
# Entrada para coordenadas manuais
entrada_manual_label = tk.Label(janela, text="Digite as coordenadas manualmente (x,y):")
entrada_manual_label.pack()
entrada_manual = tk.Entry(janela)
entrada_manual.pack()

# Botão para verificar coordenadas manualmente
verificar_manual_botao = tk.Button(janela, text="Verificar Manualmente", command=verificar_coordenada_manualmente)
verificar_manual_botao.pack()

# Rótulo para exibir o resultado da verificação manual
resultado_manual = tk.Label(janela, text="")
resultado_manual.pack()

# Entrada para o caminho do arquivo Excel
entrada_label = tk.Label(janela, text="Selecione o arquivo Excel:")
entrada_label.pack()
...
# Botão para processar as coordenadas
processar_botao = tk.Button(janela, text="Processar arquivo(s) excel", command=processar_coordenadas)
processar_botao.pack()

# Listbox para mostrar os arquivos selecionados
listbox_arquivos = tk.Listbox(janela, selectmode=tk.MULTIPLE, width=50)
listbox_arquivos.pack()

# Rótulo para exibir o resultado do processamento
